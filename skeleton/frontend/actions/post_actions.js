...
export const createPost = (post) => {
  return dispatch => {
    return  PostApiUtil.createPost(post).then(postReceived => {
      return dispatch({type: RECEIVE_POST, post: postReceived})
    });
  };
};
...
export const updatePost = (post) => {
  return dispatch => {
    return  PostApiUtil.updatePost(post).then(postReceived => {
      return dispatch({type: RECEIVE_POST, post: postReceived})
    });
  };
};
